...
describe("GET /api/podcast/trending", () => {
  ...
  describe("given zero URL parameters", () => {
    ...
    test("should fetch more podcast entries until requested count is reached (after filtering junk podcasts)", async () => {
      const firstApiPodcastResponse =
        PODCAST_TRENDING_DEFAULT_TEN_ENTRIES.feeds.slice(0, 5)
      const secondApiPodcastResponse =
        PODCAST_TRENDING_DEFAULT_TEN_ENTRIES.feeds.slice(5)

      server.use(
        createTrendingPodcastHandler({
          responses: [firstApiPodcastResponse, secondApiPodcastResponse],
        })
      )

      const podcastData = [
        ...sortExpectedPodcastsByScore(
          firstApiPodcastResponse.map((p) => ({
            id: p.id,
            url: p?.url,
            title: p.title,
            description: getSanitizedHtmlText(p.description || ""),
        ...
        ...sortExpectedPodcastsByScore(
          secondApiPodcastResponse.map((p) => ({
            id: p.id,
            url: p?.url,
            title: p.title,
            description: getSanitizedHtmlText(p.description || ""),
